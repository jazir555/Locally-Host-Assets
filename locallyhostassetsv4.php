...
if ( ! class_exists( 'SelfHostAssetsPlugin\SelfHostAssets' ) ) {
    ...
    class SelfHostAssets {
        ...
        // Rate Limiting: Maximum tasks processed per cron run
        ...
        private function __construct() {
            global $wpdb;
            $this->log_table     = $wpdb->prefix . 'self_host_assets_logs';
            $this->mapping_table = $wpdb->prefix . 'self_host_assets_mapping';
        ...
        private function init_hooks() {
            ...
            add_action( 'update_option_cron_schedule', [ $this, 'reschedule_cron_event' ], 10, 3 );

            // Enqueue admin styles for settings pages
            ...
            // Initialize background process
            add_action( 'plugins_loaded', [ $this, 'initialize_background_process' ] );
        ...
        public function cron_process_resources() {
            ...
            // Reset the processed tasks count at the start of the run
            delete_transient( 'self_host_assets_processed_tasks' );
        ...
        public function initialize_background_process() {
            if ( isset( $this->background_process ) && $this->background_process instanceof Asset_Process ) {
                $this->background_process->hook();
            }
        }
        ...
        private function process_all_styles( $force_refresh ) {
            ...
            foreach ( $wp_styles->queue as $handle ) {
                ...
                // Queue the stylesheet for background processing
                if ( isset( $this->background_process ) && $this->background_process instanceof Asset_Process ) {
        ...
        private function process_all_scripts( $force_refresh ) {
            ...
            foreach ( $wp_scripts->queue as $handle ) {
                ...
                // Queue the script for background processing
                if ( isset( $this->background_process ) && $this->background_process instanceof Asset_Process ) {
                    $this->background_process->push_to_queue( [
                        'type'          => 'js',
                        'url'           => $src,
         ...
         */
        public function download_and_replace_css( $url, $force_refresh = false ) {
         ...
         * @param string $type                 The type of the file ('css', 'js', 'font').
         ...
         * @param string $type The type of the asset ('css', 'js', 'font').
         *
        ...
        private function process_import_statements( $css_content, $css_url, &$processed_urls, $force_refresh, $current_depth ) {
            ...
            if ( preg_match_all( $pattern, $css_content, $matches, PREG_SET_ORDER ) ) {
                foreach ( $matches as $match ) {
                    ...
                    // Queue the imported CSS for background processing
                    if ( isset( $this->background_process ) && $this->background_process instanceof Asset_Process ) {
                        $this->background_process->push_to_queue( [
                            'type'          => 'css',
                    ...
                    // Dispatch the queue immediately to handle imports promptly
                    if ( isset( $this->background_process ) && $this->background_process instanceof Asset_Process ) {
                        $this->background_process->save()->dispatch();
                    }

                    // Retrieve the local URL from the mapping
         ...
         * @param string $font_dir             The directory to save font files.
         * @param string $font_url_dir         The base URL directory for font files.
         * @param int    $cache_expiration_days Cache expiration time in days.
         * @param bool   $force_refresh        Whether to force refresh cached files.
         *
        ...
        public function add_custom_cron_schedule( $schedules ) {
            // Example: Add a weekly schedule
            $schedules['weekly'] = [
                'interval' => WEEK_IN_SECONDS,
                'display'  => __( 'Weekly', 'self-host-assets' ),
            ];
            return $schedules;
        }
        ...
        public function render_manage_assets_page() {
            ...
            if ( isset( $_POST['delete_asset'] ) && isset( $_POST['asset_type'] ) && isset( $_POST['asset_url'] ) ) {
                ...
                if ( ! isset( $_POST['manage_assets_nonce'] ) || ! wp_verify_nonce( $_POST['manage_assets_nonce'], 'manage_assets_action' ) ) {
                    ...
                    // Retrieve the hashed filename from the mapping table
                        ...
                        if ( file_exists( $file_path ) ) {
                            if ( $wpdb->delete(
                                ...
                                if ( $wp_filesystem->delete( $file_path ) ) {
                                    $this->log_error( sprintf( __( 'Successfully deleted %s asset: %s', 'self-host-assets' ), strtoupper( $asset_type ), esc_url_raw( $asset_url ) ) );
                                } else {
                                    $this->log_error( sprintf( __( 'Failed to delete %s asset file: %s', 'self-host-assets' ), strtoupper( $asset_type ), esc_url_raw( $file_path ) ) );
                                }
                            } else {
                                $this->log_error( sprintf( __( 'Failed to delete mapping for asset: %s', 'self-host-assets' ), esc_url_raw( $asset_url ) ) );
                            }
                        } else {
        ...
        public function render_error_logs_page() {
            ...
            if ( isset( $_POST['delete_logs'] ) ) {
                ...
                if ( ! isset( $_POST['delete_logs_nonce'] ) || ! wp_verify_nonce( $_POST['delete_logs_nonce'], 'delete_logs_action' ) ) {
                    ...
                    if ( false !== $deleted ) {
                        $this->log_error( __( 'Successfully deleted all error logs.', 'self-host-assets' ) );
                    } else {
        ...
        public function reschedule_cron_event( $option_name, $old_value, $new_value ) {
            if ( 'cron_schedule' === $option_name && $old_value !== $new_value ) {
                $timestamp = wp_next_scheduled( 'self_host_assets_cron_event' );
                if ( $timestamp ) {
                    wp_unschedule_event( $timestamp, 'self_host_assets_cron_event' );
                }
        ...
        private function log_error( $message ) {
            if ( count( $this->errors ) < 5 ) { // Limit stored errors to prevent memory issues
                $this->errors[] = $message;
            }
        ...
        public static function uninstall() {
            ...
            // Delete logs table
            $wpdb->query( "DROP TABLE IF EXISTS {$instance->log_table}" );

            // Delete mapping table
            $wpdb->query( "DROP TABLE IF EXISTS {$instance->mapping_table}" );
        }

        /**
         * Get all cached assets from the filesystem.
         *
            ...
            foreach ( $types as $type ) {
                ...
                if ( is_dir( $dir ) ) {
                    ...
                    foreach ( $files as $file ) {
                        if ( in_array( $file, [ '.', '..', '.self-host-assets' ], true ) ) {
                            continue;
                        }

                        $file_path = trailingslashit( $dir ) . $file;
                        if ( is_file( $file_path ) ) {
                            // Retrieve original URL from mapping table
                            $original_url = $wpdb->get_var(
                                $wpdb->prepare(
                                    "SELECT original_url FROM {$this->mapping_table} WHERE hashed_filename = %s AND type = %s LIMIT 1",
                                    $file,
                                    $type
                                )
                            );

                            $local_url    = add_query_arg( 'ver', filemtime( $file_path ), trailingslashit( $url_dir ) . $file );
                            $last_updated = date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), filemtime( $file_path ) );

                            $assets[] = (object) [
                                'type'         => $type,
                                'original_url' => $original_url ? $original_url : __( 'Unknown', 'self-host-assets' ),
                                'local_url'    => $local_url,
        ...
        public function add_settings_fields() {
            // This function can be used if you want to add more fields in the future.
        }

        /**
         * Add custom admin styles for the plugin settings pages.
         */
        public function enqueue_admin_styles( $hook_suffix ) {
            if ( strpos( $hook_suffix, 'self-host-assets' ) === false ) {
                return;
            }

            wp_enqueue_style(
                'self-host-assets-admin',
                plugin_dir_url( __FILE__ ) . 'self-host-assets-admin.css',
                [],
                '1.0.0'
            );
        }
         ...
         * @param string $message The error message.
         */
        private function log_error_duplicate( $message ) {
            // This function is intentionally left empty to prevent duplication.
            // It exists because the original code had duplicate log_error and display_admin_notices methods.
        }
    ...
    class Asset_Process {
        ...
        /**
         * Hook the background process.
         */
        public function hook() {
            // Hook into WP cron to process the queue
            add_action( $this->action, [ $this, 'process_queue' ] );
        }
        ...
        public function save() {
            update_option( 'wp_background_process_' . $this->action, $this->queue );
            $this->queue = [];
            return $this;
        }
        ...
        public function dispatch() {
            if ( ! wp_next_scheduled( $this->action ) ) {
                wp_schedule_single_event( time(), $this->action, [] );
            }
        }
        ...
        public function process_queue() {
            $this->queue = get_option( 'wp_background_process_' . $this->action, [] );

            $processed_tasks = get_transient( 'self_host_assets_processed_tasks' ) ?: 0;
            $remaining_tasks = SelfHostAssets::MAX_TASKS_PER_RUN - $processed_tasks;

            if ( $remaining_tasks <= 0 ) {
                // Re-schedule the cron event for the next run
                $this->schedule_next_run();
                return;
            }

            $tasks_to_process = array_slice( $this->queue, 0, $remaining_tasks );
            $this->queue = array_slice( $this->queue, $remaining_tasks );

            foreach ( $tasks_to_process as $item ) {
                $this->task( $item );
            }
            ...
            // Re-schedule if there are still tasks left
            if ( ! empty( $this->queue ) ) {
                $this->schedule_next_run();
            }
        }
        ...
        protected function task( $item ) {
            ...
            switch ( $type ) {
                case 'css':
                    $this->main_plugin->download_and_replace_css( $url, $force_refresh );
                    break;
                case 'js':
                ...
                default:
                    // Unsupported type
                    $this->main_plugin->log_error( sprintf( __( 'Unsupported asset type: %s', 'self-host-assets' ), esc_html( $type ) ) );
                    return false;
            }

            // Rate Limiting: Increment processed tasks count
            $processed_tasks = get_transient( 'self_host_assets_processed_tasks' ) ?: 0;
            $processed_tasks++;
            set_transient( 'self_host_assets_processed_tasks', $processed_tasks, MINUTE_IN_SECONDS );
        ...
        private function schedule_next_run() {
            if ( ! wp_next_scheduled( $this->action ) ) {
                wp_schedule_single_event( time() + 60, $this->action, [] );
            }
        }
    }
     ...
     *
     * This is a simplified version of the WP Background Processing library.
     * For more robust solutions, consider using the official library.
     */
}
